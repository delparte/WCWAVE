...
#Calculate air temperature averages over the 3 hour time period (ignoring "no-data" values: -999 etc.)
#and join to stations_wElevation
arcpy.AddMessage("Calculating average air temperatures")
arcpy.MakeTableView_management(data_table, "table1", "air_temperature > -500")
arcpy.Statistics_analysis("table1", "in_memory/tableAverage", "air_temperature MEAN", "site_key")
arcpy.JoinField_management(fcStations_wElevation, "Site_Key", "in_memory/tableAverage", "site_key", "MEAN_air_temperature")
...
#Run ordinary least squares on station_wElevations
arcpy.AddMessage("Running ordinary least squares")
arcpy.CreateTable_management(scratchGDB, "coef_table")
ols = arcpy.OrdinaryLeastSquares_stats(fcStations_wElevation,"Unique_ID","in_memory/fcResid","MEAN_air_temperature","RASTERVALU", scratchGDB + "/coef_table","","")
...
arcpy.AddMessage("Estimating air temperature at all stations")
cursor = arcpy.UpdateCursor(fcStations_wElevation)
for row in cursor:
    if str(row.getValue("MEAN_air_temperature")) == "None":
        row.setValue("MEAN_air_temperature", (row.getValue("RASTERVALU") * slope) + intercept)
...
arcpy.EmpiricalBayesianKriging_ga(in_features=fcStations_wElevation, z_field="MEAN_air_temperature", out_ga_layer="#", \
    out_raster=outRaster, cell_size=output_cell_size, transformation_type="EMPIRICAL", max_local_points="100", \
