...
arcpy.AddMessage("Calculating average snow depths")
arcpy.MakeTableView_management(data_table, "table1", "snow_depth > -500")
arcpy.Statistics_analysis("table1", "in_memory/tableAverage", "snow_depth MEAN", "site_key")
arcpy.JoinField_management(fcStations_wElevation, "Site_Key", "in_memory/tableAverage", "site_key", "MEAN_snow_depth")
...
ols = arcpy.OrdinaryLeastSquares_stats(fcStations_wElevation,"Unique_ID","in_memory/fcResid","MEAN_snow_depth","RASTERVALU", scratchGDB + "/coef_table","","")
...
arcpy.AddMessage("Estimating snow depths at all stations")
cursor = arcpy.UpdateCursor(fcStations_wElevation)
for row in cursor:
    if str(row.getValue("MEAN_snow_depth")) == "None":
        row.setValue("MEAN_snow_depth", (row.getValue("RASTERVALU") * slope) + intercept)
...
arcpy.EmpiricalBayesianKriging_ga(in_features=fcStations_wElevation, z_field="MEAN_snow_depth", out_ga_layer="#", \
    out_raster=scratchGDB + "/raster1", cell_size=output_cell_size, transformation_type="EMPIRICAL", max_local_points="100", \
    overlap_factor="1", number_semivariograms="100", \
...
#Replace negative snow depth values with 0
arcpy.gp.Con_sa(scratchGDB + "/raster1", 0.0, outRaster, scratchGDB + "/raster1", "Value < 0")

arcpy.Delete_management(scratchGDB + "/raster1")