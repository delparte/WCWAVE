...
arcpy.Statistics_analysis("table1", "in_memory/tableAverage", "ppts MEAN", "site_key")
arcpy.JoinField_management(fcStations_wElevation, "Site_Key", "in_memory/tableAverage", "site_key", "MEAN_ppts")
...
ols = arcpy.OrdinaryLeastSquares_stats(fcStations_wElevation,"Unique_ID","in_memory/fcResid","MEAN_ppts","RASTERVALU", scratchGDB + "/coef_table","","")
...
arcpy.AddMessage("Estimating precipitation at all stations")
cursor = arcpy.UpdateCursor(fcStations_wElevation)
for row in cursor:
    if str(row.getValue("MEAN_ppts")) == "None":
        row.setValue("MEAN_ppts", (row.getValue("RASTERVALU") * slope) + intercept)
...
arcpy.EmpiricalBayesianKriging_ga(in_features=fcStations_wElevation, z_field="MEAN_ppts", out_ga_layer="#", out_raster=outRaster, \
